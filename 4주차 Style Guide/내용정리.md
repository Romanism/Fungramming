...
|함수|인수 단위를 포함하게 재작성|
|--|--|
|Start(int **delay**)|delay $\rightarrow$ **delay_secs**|
|Create(int **size**)|size $\rightarrow$ **size_mb**|
|ThrottleDownload(float **limit**)|limit $\rightarrow$ **max_kbps**|
|Rotate(float **angle**)|angle $\rightarrow$ **degrees_cw**|
...
##### 1) 코드를 읽는 사람이 이미 친숙한, 일관성 있는 레이아웃을 사용하기
